...
// Host imports (for delegation to underlying WASI implementation)
use bindings::wasi::cli::stdin::get_stdin as host_get_stdin;
use bindings::wasi::cli::stdout::get_stdout as host_get_stdout;
use bindings::wasi::cli::stderr::get_stderr as host_get_stderr;
use bindings::wasi::io::streams::InputStream as HostInputStream;
...
// CLI export Guest traits (for implementing stdin/stdout/stderr delegation)
...
fn convert_stream_error(err: HostStreamError) -> StreamError {
    match err {
        HostStreamError::LastOperationFailed(_) => {
            StreamError::LastOperationFailed(Error::new(StreamErrorResource::new("last operation failed")))
        }
        HostStreamError::Closed => StreamError::Closed,
    }
}
...
// ============================================================================
// Host stdin/stdout/stderr delegation wrappers
// ============================================================================

/// Wrapper that delegates to host stdin
pub struct HostInputStreamWrapper {
    inner: HostInputStream,
}

impl HostInputStreamWrapper {
    fn new(inner: HostInputStream) -> Self {
        Self { inner }
    }
}

impl GuestInputStream for HostInputStreamWrapper {
    fn read(&self, len: u64) -> Result<Vec<u8>, StreamError> {
        self.inner.read(len).map_err(convert_stream_error)
    }

    fn blocking_read(&self, len: u64) -> Result<Vec<u8>, StreamError> {
        self.inner.blocking_read(len).map_err(convert_stream_error)
    }

    fn skip(&self, len: u64) -> Result<u64, StreamError> {
        self.inner.skip(len).map_err(convert_stream_error)
    }

    fn blocking_skip(&self, len: u64) -> Result<u64, StreamError> {
        self.inner.blocking_skip(len).map_err(convert_stream_error)
    }
...
/// Wrapper that delegates to host output streams
pub struct HostOutputStreamWrapper {
    inner: HostOutputStream,
}

impl HostOutputStreamWrapper {
    fn new(inner: HostOutputStream) -> Self {
        Self { inner }
    }
}

impl GuestOutputStream for HostOutputStreamWrapper {
    fn check_write(&self) -> Result<u64, StreamError> {
        self.inner.check_write().map_err(convert_stream_error)
    }

    fn write(&self, contents: Vec<u8>) -> Result<(), StreamError> {
        self.inner.write(&contents).map_err(convert_stream_error)
    }

    fn blocking_write_and_flush(&self, contents: Vec<u8>) -> Result<(), StreamError> {
        self.inner.blocking_write_and_flush(&contents).map_err(convert_stream_error)
    }

    fn flush(&self) -> Result<(), StreamError> {
        self.inner.flush().map_err(convert_stream_error)
    }

    fn blocking_flush(&self) -> Result<(), StreamError> {
        self.inner.blocking_flush().map_err(convert_stream_error)
    }
    ...
    fn write_zeroes(&self, len: u64) -> Result<(), StreamError> {
        self.inner.write_zeroes(len).map_err(convert_stream_error)
    }

    fn blocking_write_zeroes_and_flush(&self, len: u64) -> Result<(), StreamError> {
        self.inner.blocking_write_zeroes_and_flush(len).map_err(convert_stream_error)
    }
...
impl StdoutGuest for FsWrapper {
    fn get_stdout() -> OutputStream {
        OutputStream::new(HostOutputStreamWrapper::new(host_get_stdout()))
    }
}
